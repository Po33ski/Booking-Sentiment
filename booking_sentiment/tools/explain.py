...
def _compute_attributions(model, tokenizer, text: str) -> Tuple[List[str], List[float], float, int]:
    ...
    model_wrapper = ModelWrapper(model_copy) # wrap the model in a callable that can be used to get the embeddings for a given input.
    input_x_gradient = InputXGradient(model_wrapper) # create an InputXGradient object that can be used to compute the attributions.    
    ...
    attributions = attributions.sum(dim=-1).squeeze(0) # sum the attributions over the embedding dimension. This returns a tensor of shape (batch_size, sequence_length).
    norm = torch.norm(attributions)
    attributions = attributions / (norm + 1e-12) # normalize the attributions. This returns a tensor of shape (batch_size, sequence_length).
    attributions = list([float(a) for a in attributions]) # convert the attributions to a list of floats. This returns a list of floats.

    with torch.no_grad():
        logits = model(**tokenizer_output).logits # get the logits for the input. This returns a tensor of shape (batch_size, num_labels).
        y_pred_proba = softmax(logits, dim=1)[0, 1].item() # get the probability of the positive class. This returns a float.
        y_pred = int(y_pred_proba >= 0.5) # get the predicted class. This returns an integer.

    # remove special tokens
    tokens = tokens[1:-1] # remove the special tokens. This returns a list of tokens.
    attributions = attributions[1:-1] # remove the special tokens. This returns a list of attributions. This returns a list of floats.      
...
def explain_samples(model_dir: Path, test_df: pd.DataFrame, out_dir: Path, num_pos: int = 3, num_neg: int = 3) -> Path:
    ...
    pos_samples = test_df[test_df["label"] == 1].head(num_pos) # get the first num_pos positive samples. This returns a pandas DataFrame.
    neg_samples = test_df[test_df["label"] == 0].head(num_neg) # get the first num_neg negative samples. This returns a pandas DataFrame.

    outputs = []
    for row in pd.concat([pos_samples, neg_samples]).itertuples(index=False): # concatenate the positive and negative samples. This returns a pandas DataFrame.
