...
def run_split(cfg: ProjectConfig) -> None:
    ...
    if cfg.dataset.sample_size:
        df_fixed = df_fixed.sample(n=cfg.dataset.sample_size, random_state=cfg.dataset.random_state, replace=False)
        df_fixed = df_fixed.reset_index(drop=True)
        typer.echo(f"[split] Subsampled to {len(df_fixed)} rows")
